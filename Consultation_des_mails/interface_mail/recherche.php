...
$recherche = $_POST['textarech'];
# Requetage
$requete= $recherche." <DANS> mail";
//$requete= "*";
...
# On envoie l'appel regroupant requete+selection a XediX
 ...
 ##################################################################################################################
   ...
   print "<HTML><HEAD><TITLE>Recherche</TITLE>";
   ...
   print "<div id ='page'>";
	...
	print "<a href = main.php>&#60;-Retour</a>";
 	print "<h1 align=center>Recherche</h1>";
 	print "</div>";
	...
	$nb = 0;
   for ( $i=0; $i < $nbvaleurs ; $i++ ) {
   	if(substr_count($fromArray[$i],$recherche)> 0 || substr_count($subjectArray[$i+1],$recherche)> 0  || 
   	substr_count($toArray[$i],$recherche)> 0 || substr_count($bodyArray[$i+1],$recherche)> 0 ){
   ...
   print "</ol>";
   print "</div>";
   print "<div id = 'footer'><a href = './recherche.php'> ^^ Haut de page ^^</a></div>";
   print "</div>";
   print "</body></html>";
